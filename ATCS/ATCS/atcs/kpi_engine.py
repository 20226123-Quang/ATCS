...
class KPIEngine:
    ...
    def compute_lane_kpis(self, lane_id: str, cycle_length_seconds: float) -> LaneKPI:
        ...
        # HCM-style decomposition:
        # d = d1 * PF + d2 + d3
        x_capped = min(v_over_c, 1.0)
        denominator = max(1.0 - x_capped * g_over_c, eps)
        d1 = 0.5 * cycle_length * ((1.0 - g_over_c) ** 2) / denominator

        overflow = max(0.0, v_over_c - 1.0)
        d2 = (
            constants.incremental_delay_k
            * (overflow ** constants.incremental_delay_power)
            * cycle_length
        )
        if v_over_c > constants.v_c_penalty_threshold:
            d2 += (
                constants.v_c_penalty_weight
                * (v_over_c - constants.v_c_penalty_threshold)
                * cycle_length
            )

        arrival_rate_pcu_per_second = stats.cycle_inflow_pcu / cycle_steps
        if arrival_rate_pcu_per_second <= eps:
            d3 = 0.0
        else:
            d3 = stats.initial_cycle_queue / max(arrival_rate_pcu_per_second, eps)

        control_delay = d1 * constants.green_wave_pf_default + d2 + d3
