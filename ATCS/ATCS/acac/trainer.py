...
class ACACTrainer:
    ...
    def collect_async_rollout(self, env, max_steps=1000):
        ...
        while not done and t < max_steps:
            ...
            for name in requiring:
                ...
                self.hidden_states[i] = self.encoders[i](
                    z_it, p_it, self.hidden_states[i]
                )
    ...
    def evaluate_model(self, env, max_steps=1000):
        ...
        while not done and t < max_steps:
            ...
            total_reward += self._global_reward_scalar(reward) * float(dt)
            obs = next_obs
            t += dt
    ...
    def update_critic(self, n_epochs=4):
        ...
        states = torch.stack([e["global_h"] for e in self.critic_buffer.buffers]).to(
            self.device
        )
        returns = (
            torch.stack(
    ...
    def update_actors(self, clip_eps=0.2, n_epochs=4):
        ...
        for i in range(self.num_agents):
            ...
            actions = torch.stack([e["raw_action"] for e in agent_entries]).to(
                self.device
            )
            advantages = torch.stack([e["advantage"] for e in agent_entries]).to(
                self.device
            )
            ...
            old_log_probs = (
                torch.stack([e["old_log_prob"] for e in agent_entries])
                .to(self.device)
                .squeeze(-1)
            )
