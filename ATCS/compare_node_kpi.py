...
def run_acac_episode(env, trainer, max_steps):
    ...
    kpis = {
        "delay": {name: [] for name in tls_names},
        "queue": {name: [] for name in tls_names},
        "saturation": {name: [] for name in tls_names},
    }
...
def plot_comparison_per_node(
    acac_kpis, fixed_kpis, scenario_name, output_dir, total_steps_count, tls_names
):

    for name in tls_names:
        acac_avg = [
            np.mean(acac_kpis["delay"][name]),
            np.mean(acac_kpis["queue"][name]),
            np.mean(acac_kpis["saturation"][name]),
        ]
        fixed_avg = [
            np.mean(fixed_kpis["delay"][name]),
            np.mean(fixed_kpis["queue"][name]),
            np.mean(fixed_kpis["saturation"][name]),
        ]
        ...
        output_path = os.path.join(
            output_dir, f"{scenario_name}_node_{name}_compare_kpi.png"
        )
...
def main():
    ...
    output_dir = Path(__file__).resolve().parents[1] / "checkpoints" / "compare_kpi" / "per_node"
